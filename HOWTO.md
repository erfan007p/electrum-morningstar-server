...
document, we assume that user is called 'morningstar'). We also assume the
morningstar user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run morningstard and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit morningstard to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running morningstard and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `morningstard` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser morningstar --disabled-password
    $ sudo apt-get install git
    $ sudo su - morningstar
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/morningstar/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download morningstard
.

If you prefer to compile morningstard, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - morningstar
    $ cd ~/src && git clone https://github.com/Morningstarpayments/morningstar 
    $ cd morningstar/src
    $ make -f makefile.unix
    $ strip src/morningstard src
    $ cp -a src/morningstard src ~/bin

### Step 3. Configure and start morningstard

In order to allow Electrum to "talk" to `morningstard`, we need to set up an RPC
username and password for `morningstard`. We will then start `morningstard` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.Morningstar
    $ $EDITOR ~/.Morningstar/Morningstar.conf

Write this in `Morningstar.conf`:
...
If you have an existing installation of morningstard and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ ./morningstard -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `morningstard`:

    $ ./morningstard

Allow some time to pass, so `morningstard` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ ./morningstard getinfo

Before starting the electrum server your morningstard should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start morningstard at boot
time, running as the 'morningstar' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/Morningstarpayments/morningstar
    $ cd electrum-morningstar-server
    $ sudo configure
...
and further instructions from the Electrum-MORNINGSTAR full archival server foundry at:
http://electrum-morningstar.space/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-morningstar.conf) when starting up. This
file includes the database setup, morningstard RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-morningstar.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-morningstar-server.
If you're using user morningstar to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "morningstar hard nofile 65536" >> /etc/security/limits.conf
     echo "morningstar soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close morningstard for incoming connections and connect outbound only

2. Consider restarting morningstard (together with electrum-morningstar-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # electrum-morningstar-server start

Note: If you want to run the server without installing it on your system, just run 'run_electrum_morningstar_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # electrum-morningstar-server stop


If your system supports it, you may add electrum-morningstar-server to the /etc/init.d directory. 
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which electrum-morningstar-server` /etc/init.d/electrum-morningstar-server
    # update-rc.d electrum-morningstar-server defaults
...
$morningStar's to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-morningstar and we'll try to congratulate you
on supporting the community by running an Electrum-MORNINGSTAR node.

If you're operating a public Electrum-MORNINGSTAR server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-morningstar-server
It'll contain announcements about important updates to Electrum-MORNINGSTAR
